...
#RF: O sistema deve permitir que um usuário se autentifique para poder obter um token
...
def login():
    try:
        ...
        return jsonify({"error": "Erro durante a requisição do dado"})
...
def create_products(token):
    ...
    return jsonify({"message": "Esta é a rota de criação de um produto",
...
def get_products_by_id(product_id):
    ...
    return jsonify({"message": f"Esta é a rota de vizualição do detalhe do id do produto {product_id}"})

#RF: O sistema deve permitir a atualização de um único produto
...
def update_product(token, product_id):
    ...
    return jsonify(ProductDBMondel(**updated_product).model_dump(by_alias=True, exclude=None))

#RF: O sistema deve permitir a deletar um úmico produto e produto existente
...
@main_bp.route('/')
def index():
    return jsonify({"message": "Bem vindo ao Teste"})
